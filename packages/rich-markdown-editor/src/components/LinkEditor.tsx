...
class LinkEditor extends React.Component<Props, State> {
  ...
  get href(): string {
    const selectionText = this.props.view.state.doc.cut(
      this.props.from,
      this.props.to
    ).textContent;
    const isSelectionUrl = isUrl(selectionText);

    if (this.props.mark && this.props.mark.attrs.href)
      return this.props.mark.attrs.href;
    if (isSelectionUrl) return selectionText;
    return "";
  }
  ...
  render() {
    ...
    const Tooltip: any = this.props.tooltip;
...
export default withTheme(LinkEditor as any);
