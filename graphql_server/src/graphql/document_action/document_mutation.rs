...
impl DocumentMutation {
    ...
    async fn document_upsert_writing_block(
        &self,
        ctx: &Context<'_>,
        page_content_id: Uuid,
        mut writing_block: WritingBlock,
    ) -> Result<WritingBlock> {
        let user_id = get_user_id_from_ctx(ctx).await?;
        let page = {
            let mut conn = get_conn_from_ctx(ctx).await?;
        ...
        let mut conn = get_conn_from_ctx(ctx).await?;
        let existing_writing_block = WritingBlock::find(&mut conn, writing_block.id).format_err();
        if let Ok(writing_block) = existing_writing_block {
            if writing_block.page_content_id != page_content_id {
                return Err(IkigaiError::new_bad_request(
                    "Cannot update writing block of other page",
                ))
                .format_err();
            }
        }

        writing_block.page_content_id = page_content_id;
        writing_block.creator_id = user_id;
        let writing_block = WritingBlock::upsert(&mut conn, writing_block).format_err()?;

        Ok(writing_block)
    }

    async fn document_clone_writing_block(
        &self,
        ctx: &Context<'_>,
        writing_block_id: Uuid,
        new_writing_block_id: Uuid,
        ...
        let (mut writing_block, old_page) = {
            let mut conn = get_conn_from_ctx(ctx).await?;
        ...
        writing_block.creator_id = get_user_id_from_ctx(ctx).await?;
        writing_block.page_content_id = new_page_content_id;
        writing_block.id = new_writing_block_id;

        let mut conn = get_conn_from_ctx(ctx).await?;
        let existing_new_writing_block = WritingBlock::find(&mut conn, new_writing_block_id);
        if existing_new_writing_block.is_ok() {
            return Err(IkigaiError::new_bad_request(
                "Writing block already existed in other pages",
            ))
            .format_err();
        }
        let writing_block = WritingBlock::upsert(&mut conn, writing_block).format_err()?;
